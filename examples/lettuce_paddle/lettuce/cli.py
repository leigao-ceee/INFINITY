...
from lettuce.utils import cuda_available
...
def main(ctx, cuda, gpu_id, precision):
    ...
    if cuda:
        ...
        device = ("cpu")
    dtype = {"half": paddle.float16, "single": paddle.float32, "double": paddle.float64}[precision]
...
def benchmark(ctx, steps, resolution, profile_out, flow, vtk_out):
    ...
    click.echo("Finished {} steps in {} bit precision. MLUPS: {:10.2f}".format(
        steps, str(dtype).replace("paddle.float32", ""), mlups))
    return 0
